...
TeamMap.del = function (grid, rowIndex, colIndex){
	Ext.Msg.confirm("提示","你确定要删除该记录么?",function(confirm){
		if("yes" == confirm){
			...
			Ext.Ajax.request({
			    url: 'sbobetTeamMap.ajax?method=deleteTeamMap',
			    params:rec.data,
...
TeamMap.store = Ext.create('Ext.data.Store',{
	...
	proxy: {
    	type: 'ajax',
        url : "sbobetTeamMap.ajax?method=listTeamMap",
        reader: {
            type: 'json',
            root: 'resultList',
...
TeamMap.gridPanel = Ext.create('Ext.grid.Panel',{
	...
	dockedItems:[{
		...
		dock: 'top',
		items:[
					'利记网球队名:',{ xtype:'textfield', name:'id|like'}
					,'-',
					'平博网球队名:',{ xtype:'textfield', name:'ps3838_team_name|like'}
		       ]
	},{
    	xtype: 'toolbar',
	    dock: 'top',
...
TeamMap.formPanel = Ext.create('Ext.form.Panel',{
	...
	items:[
	     {fieldLabel:'利记网球队名',xtype:'textfield',name:'id',allowBlank:false},        
	     {fieldLabel:'平博网球队名',xtype:'textfield',name:'ps3838_team_name',allowBlank:false},        
